...
from visualizer.graphs_and_stats_new_2 import triptych_graphs, quadrivium_graphs, single_graph
...
if __name__ == "__main__":
    ...
    res_errors_se2 = np.load(os.path.join(path_to_shared_results, 'errors_se2_5.npy'))
    res_times_se2 = np.load(os.path.join(path_to_shared_results, 'comp_time_se2_5.npy'))

    with open(os.path.join(path_to_shared_results, 'exp_methods_table_se2_5'), 'rb') as f:
        res_exp_methods_param_se2 = pickle.load(f)

    with open(os.path.join(path_to_shared_results, 'exp_methods_param_se2_5'), 'rb') as f:
        res_transf_param_se2 = pickle.load(f)

    # HOM #
    res_times_hom = np.load(os.path.join(path_to_shared_results, 'comp_time_hom_5.npy'))
    res_errors_hom  = np.load(os.path.join(path_to_shared_results, 'errors_hom_5.npy'))
    ...
    with open(os.path.join(path_to_shared_results, 'exp_methods_table_gauss__6'), 'rb') as f:
        res_exp_methods_param_gauss = pickle.load(f)

    # Real #
    mean_res_errors_real = np.load(os.path.join(path_to_shared_results, 'f_mean_errors_real_5.npy'))
    mean_res_times_real  = np.load(os.path.join(path_to_shared_results, 'f_mean_times_real_5.npy'))
    mean_res_stdev_real  = np.load(os.path.join(path_to_shared_results, 'f_stdev_real_5.npy'))

    with open(os.path.join(path_to_shared_results, 'f_transformation_parameters_real_5'), 'rb') as f:
        res_transf_param_real = pickle.load(f)

    with open(os.path.join(path_to_shared_results, 'f_methods_real_5'), 'rb') as f:
        res_exp_methods_param_real = pickle.load(f)
    ...
    if see_hom:
        ...
        single_graph(selected_time_hom,
                     selected_error_hom,
    ...
    if see_gauss:
        ...
        print res_exp_methods_param_gauss
        print visualize_methods_gauss

        ### Gauss - 1 ###
        #  Table methods, steps, plot color, line-style, marker.
    ...
    if see_real:
        ...
        '''
        If the mean is directly loaded:
        mean_error_real = np.mean(res_errors_real, axis=2)
        mean_time_real = np.mean(res_times_real, axis=2)
        '''

        selected_error_real = np.array([mean_res_errors_real[i, :] for i in index_methods_considered_real])

        selected_time_real = np.array([mean_res_times_real[i, :] for i in index_methods_considered_real])
