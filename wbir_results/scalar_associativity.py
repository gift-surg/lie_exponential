...
from path_manager import pfo_results, pfo_notes_figures, pfo_notes_tables, \
    displacements_folder_path_AD, pfo_notes_sharing
...
if __name__ == "__main__":
    ...
    if compute:  # or compute or load
        ...
        if save_for_sharing:
            ...
            np.save(os.path.join(pfo_notes_sharing, 'exp_scalar_associativity_errors_real' + file_suffix), errors)

            with open(os.path.join(pfo_notes_sharing, 'exp_scalar_associativity_parameters_real'+file_suffix), 'wb') as f:
                pickle.dump(parameters, f)

            with open(os.path.join(pfo_notes_sharing, 'exp_scalar_associativity_methods_table_real'+file_suffix), 'wb') as f:
                pickle.dump(methods, f)
